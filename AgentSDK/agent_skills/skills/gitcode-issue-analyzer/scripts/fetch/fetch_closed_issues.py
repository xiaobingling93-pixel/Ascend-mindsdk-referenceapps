...
def main():
    if len(sys.argv) < 2:
        ...
        logger.info("Usage: python fetch_closed_issues.py <repo>")
        logger.info("Example: python fetch_closed_issues.py mind-cluster")
        return 1
    ...
    logger.info(f"Retrieving {repo} closed issues from repository...")
    ...
    try:
        ...
        while True:
            ...
            issues = fetch_issues(git_code_token, repo, "closed", page)
        ...
        logger.info(f"Total closed issues retrieved: {len(all_issues)}")

        if all_issues:
            df_closed = pd.DataFrame(all_issues)
            output_file = f'{repo}_closed_issue.xlsx'
            df_closed.to_excel(output_file, index=False, engine='openpyxl')
            logger.info(f"Successfully saved {output_file}")
        else:
            logger.warning("No closed issues found")

    except Exception as e:
        logger.error(f"Failed to retrieve closed issues: {e}")
        return 1
